...
const FtpDeployer = function () {
	...
	function canIncludeFile(filePath) {
		...
		if (include.length > 0) {
			for (i = 0; i < include.length; i++)
				return minimatch(filePath, include[i], { matchBase })
		}

		if (exclude.length > 0) {
			for (i = 0; i < exclude.length; i++)
				return !minimatch(filePath, exclude[i], { matchBase })
		}
	...
	function ftpPut(partialFilePath, cb) {
		...
		const
			fullLocalPath = path.join(localRoot, partialFilePath),
			emitData = {
	...
	function ftpMakeDirectoriesIfNeeded(cb) {
		async.eachSeries(partialDirectories, ftpMakeRemoteDirectoryIfNeeded, cb)
	}
